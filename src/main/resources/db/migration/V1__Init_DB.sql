...
create table status (discount integer, id bigserial not null, name varchar(255), primary key (id));
alter table if exists bike add constraint FKt9wuhid45wo6chotxfh35oikf foreign key (owner_id) references customer;
alter table if exists customer add constraint FKniyp1iwu4ooty4hngldujnlon foreign key (status_id) references status;
